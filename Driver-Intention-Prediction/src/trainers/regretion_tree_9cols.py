# -*- coding: utf-8 -*-
"""
Classical baselines (9 columns): Logistic Regression and Decision Tree.

Data geometry:
 - expanded_data.csv : [samples*steps, 9]   (flattened sequences)
...
# -----------------------------
X_raw = np.loadtxt("expanded_data.csv", delimiter=",")  # [steps*samples, 9]
...
n_features = X_raw.shape[1]            # expected: 9
flat_dim = steps * n_features          # e.g., 20 * 9 = 180

X = X_raw.reshape(n_samples, flat_dim) # [samples, 180]
...
# -----------------------------
# Logistic Regression (multinomial or OVR)
# -----------------------------
logreg = LogisticRegression(
    multi_class='ovr',    # OVR to be robust with limited data; 'multinomial' is also possible
    solver='liblinear',   # 'lbfgs' for multinomial; 'liblinear' works well for OVR
    max_iter=200
)
...
tree = DecisionTreeClassifier(
    ...
    max_depth=None,       # let it grow; consider pruning in practice
    random_state=42
)
tree.fit(X_tr, y_tr)
y_pred_tree = tree.predict(X_te)
# Some sklearn trees expose predict_proba for ROC-AUC; if disabled by params, handle with try/except:
try:
...
def report_block(name, y_true, y_pred, y_proba=None):
    ...
    plt.title(f"Confusion Matrix - {name} (9 cols)")
    plt.tight_layout()
    plt.savefig(f"confusion_{name.lower().replace(' ', '_')}_9cols.png", dpi=150, bbox_inches='tight')
    plt.close()

    # Macro ROC-AUC (OVR) if probabilities are available
    if y_proba is not None:
        # Binarize integer labels for OVR AUC
